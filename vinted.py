...
async def sub(ctx, vintedurl):
    ...
    with open("config.json", 'w+') as configedit:
        configs["suburl"][str(x.url)] = {"url": str(vintedurl), "salon": str(ctx.channel.name)}
...
async def change_url(ctx, new_url):
    for weburl, data in configs["suburl"].items():
        if data["salon"] == ctx.channel.name:
            with open("config.json", 'w+') as configedit:
...
async def remove_sub(ctx):
    webhook = next((weburl for weburl, data in configs["suburl"].items() if data["salon"] == ctx.channel.name), None)
    if webhook:
        with open("config.json", 'w+') as configedit:
        ...
        await ctx.send(f"{ctx.author.mention} - **❌ Aucun lien trouvé pour ce salon !**")

@bot.command()
async def change_color_text(ctx, color):
    if color not in ["fix", "YAML"]:
        await ctx.send(f"{ctx.author.mention} - **❌ La couleur n'est pas disponible ! Essayez autre chose.**")
        return

    for data in configs["suburl"].values():
        if data["salon"] == ctx.channel.name:
            with open("config.json", 'w+') as configedit:
                configs["embed-color-text"] = color
                json.dump(configs, configedit, indent=4)
            await ctx.send(f"{ctx.author.mention} - **✔️ La couleur du text de l'embed a été modifiée avec succès !**")
            return
    ...
    for data in configs["suburl"].values():
        if data["salon"] == ctx.channel.name:
            with open("config.json", 'w+') as configedit:
                ...
                json.dump(configs, configedit, indent=4)
            await ctx.send(f"{ctx.author.mention} - **✔️ La couleur de l'embed a été modifiée avec succès !**")
            return
